...
#pragma warning restore
    ...
    [SerializeField, HideInInspector] private OVRManager.ColorSpace _colorGamut = OVRManager.ColorSpace.Rift_CV1;
    ...
    /// </summary>
    [RangeAttribute(0.5f, 2.0f)] [Tooltip("Min RenderScale the app can reach under adaptive resolution mode")]
    ...
    /// </summary>
    [RangeAttribute(0.5f, 2.0f)] [Tooltip("Max RenderScale the app can reach under adaptive resolution mode")]
    ...
    [Header("Tracking")] [SerializeField] [Tooltip("Defines the current tracking origin type.")]
    ...
    private void InitOVRManager()
    {
        ...
        //Debug.Log("Unity v" + Application.unityVersion + ", " +"Oculus Utilities v" + OVRPlugin.wrapperVersion + ", " +"OVRPlugin v" + OVRPlugin.version + ", " +"SDK v" + OVRPlugin.nativeSDKVersion + ".");

        ////Debug.Log.LogFormat("SystemHeadset {0}, API {1}", systemHeadsetType.ToString(), xrApi.ToString());

        if (xrApi == XrApi.OpenXR)
        {
            ////Debug.Log.LogFormat("OpenXR instance 0x{0:X} session 0x{1:X}", xrInstance, xrSession);
        }

#if !UNITY_EDITOR
        if (IsUnityAlphaOrBetaVersion())
        {
            ////Debug.Log.LogWarning(UnityAlphaOrBetaVersionWarningMessage);
        }
#endif
        ...
        enableMixedReality =
 false;     // we should never start the standalone game in MxR mode, unless the command-line parameter is provided
#endif
        ...
        // //Debug.LogFormat("Current display frequency {0}, available frequencies [{1}]",display.displayFrequency, string.Join(", ", display.displayFrequenciesAvailable.Select(f => f.ToString()).ToArray()));
